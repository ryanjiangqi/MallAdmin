...
class Mybootstrap extends Paginator {
    ...
    protected function getLinks() {
        ...
        //根据$side 设置中间的页码数，如 $side=1,则中间页码为3，$side=2,则中间页面为5
        $side = 3;
        $window = $side * 2;
    ...
    public function render() {
        if ($this->hasPages()) {
            if ($this->simple) {
                return sprintf(
                        '%s<div class="pagination">%s %s %s</div>', $this->css(), $this->prev(), $this->getLinks(), $this->next()
                );
            } else {
                return sprintf(
                        '%s<div class="pagination">%s %s %s %s %s %s</div>', $this->css(), $this->home(), $this->prev(), $this->getLinks(), $this->next(), $this->last(), $this->info()
                );
            }
        }
    }
